...
/**
 * Thêm một hoặc nhiều chủ đề mới vào Google Sheet (tự động căn theo cấu trúc cột của sheet)
 */
export async function appendTopicsToSheet({
  spreadsheetId = null,
  sheetName = 'topics',
  topics = [],
}) {
  ...
  // Đọc dữ liệu hiện có để lấy headers và ID lớn nhất
  const readUrl = `https://sheets.googleapis.com/v4/spreadsheets/${sid}/values/${encodeURIComponent(sname)}!A1:Z`;
  const rRes = await fetch(readUrl, { headers: { Authorization: `Bearer ${token}` } });
  const rData = await rRes.json();
  if (rData.error) {
    throw new Error(`Google Sheets API Error (${rData.error.code}): ${rData.error.message}`);
  }

  let rows = rData.values || [];
  let headers = [];
  if (rows.length === 0) {
    headers = ['id', 'topic', 'category', 'keywords', 'status', 'created_at', 'published_at', 'batch', 'post_link'];
    await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${sid}/values/${encodeURIComponent(sname)}!A1?valueInputOption=USER_ENTERED`, {
      method: 'PUT',
  ...
  const topicColIdx = headers.indexOf('topic');
  const categoryColIdx = headers.indexOf('category');
  const keywordsColIdx = headers.indexOf('keywords');
  const statusColIdx = headers.indexOf('status');
  const createdAtColIdx = headers.indexOf('created_at');
  const publishedAtColIdx = headers.indexOf('published_at');
  const batchColIdx = headers.indexOf('batch');
  const postLinkColIdx = headers.indexOf('post_link');
  const slotColIdx = headers.indexOf('slot');
  const dayColIdx = headers.indexOf('day');

  // Tìm ID số lớn nhất hiện tại
  let maxId = 0;
  if (idColIdx !== -1) {
    for (let i = 1; i < rows.length; i++) {
      const val = parseInt(rows[i][idColIdx], 10);
  ...
  if (maxId === 0) {
    maxId = Math.max(0, rows.length - 1);
  }
  ...
  for (let idx = 0; idx < topics.length; idx++) {
    const item = typeof topics[idx] === 'string' ? { topic: topics[idx] } : topics[idx];
    const newId = maxId + idx + 1;
    const row = new Array(headers.length).fill('');

    if (idColIdx !== -1) row[idColIdx] = String(newId);
    if (topicColIdx !== -1) row[topicColIdx] = item.topic || '';
    if (categoryColIdx !== -1) row[categoryColIdx] = item.category || 'Giải pháp';
    if (keywordsColIdx !== -1) row[keywordsColIdx] = item.keywords || '';
    if (statusColIdx !== -1) row[statusColIdx] = item.status || 'pending';
    if (createdAtColIdx !== -1) row[createdAtColIdx] = todayStr;
    if (publishedAtColIdx !== -1) row[publishedAtColIdx] = '';
    if (batchColIdx !== -1) row[batchColIdx] = item.batch || 'ai_auto';
    if (postLinkColIdx !== -1) row[postLinkColIdx] = '';
    if (slotColIdx !== -1) row[slotColIdx] = item.slot || '';
    if (dayColIdx !== -1) row[dayColIdx] = item.day || '';

    rowsToAppend.push(row);
    addedList.push({
      id: String(newId),
      topic: item.topic,
      category: item.category || 'Giải pháp',
      keywords: item.keywords || '',
      status: item.status || 'pending',
    });
  }

  const appendUrl = `https://sheets.googleapis.com/v4/spreadsheets/${sid}/values/${encodeURIComponent(sname)}!A:Z:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`;
  ...
  if (appendData.error) {
    throw new Error(`Google Sheets Append Error (${appendData.error.code}): ${appendData.error.message}`);
  }

  console.log(`[GoogleSheet] ✅ Đã nạp thành công ${rowsToAppend.length} chủ đề mới vào sheet [${sname}]!`);
  return {
    ok: true,
    sheetName: sname,
