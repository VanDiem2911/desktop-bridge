...
export function loadScheduleConfig() {
  ...
  return {
    ...
    scheduleTimes: ['08:00', '09:30', '14:00', '16:00', '20:00'],
    channels: { fanpage: true, groups: true },
    aspectRatio: '16:9',
    hasMascotDu: true,
    topics: [],
    lastRunAt: null,
...
export async function generateContentWithGroq(topic = null, config = null, isFallback = false) {
  ...
  const mascotInstructions = hasMascotDu
    ? 'Preserve the complete 3D scene, background environment, mascot outfit, and card from the uploaded reference image. Only specify the Vietnamese headline and subheadline for the card.'
    : `
...
`;
  ...
  const promptWithText = hasMascotDu
    ? [
        `TIÊU ĐỀ CHÍNH: "${headlineText}"`,
        subheadlineText ? `PHỤ ĐỀ: "${subheadlineText}"` : '',
      ].filter(Boolean).join('\n')
    : [
        parsed.imagePrompt,
 ...
 */
export async function generateImageWithChatGPT(imagePrompt, aspectRatio = '16:9', hasMascotDu = true, preferredAccount = null) {
  try {
    console.log('[AutoPilot] Đang gửi yêu cầu tạo ảnh ChatGPT tới Bridge Port 3001...');
    const cleanRatio = (aspectRatio === '4:5' || aspectRatio === '4/5' || !aspectRatio) ? '16:9' : aspectRatio;
...
export async function publishToFacebookChannels({ caption, imageBase64, channels = {}, accounts = {} }) {
  ...
  // 2. Facebook Groups (Port 3002) - Đăng bài trong nhóm
  if (channels.groups) {
    try {
      console.log('[AutoPilot] Đang đăng bài lên Facebook Groups (Port 3002)...');
      const res = await fetch('http://127.0.0.1:3002/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      results.push({
        channel: 'groups',
        channelName: 'Facebook Groups (Nhóm)',
        success: Boolean(data.ok),
        postUrl: data.postUrl || data.postUrls?.[0] || null,
        publishedAt: data.completedAt || new Date().toISOString(),
        error: data.ok ? null : data.error || 'Lỗi đăng Nhóm Facebook',
      });
    } catch (err) {
      results.push({
        channel: 'groups',
...
export async function runAutoPilotCycle(optionsOrTopic = null) {
  ...
  const effectiveChannels = customChannels || config.channels || { fanpage: true, groups: true };
  ...
  console.log(`Kênh hoạt động: Fanpage: ${Boolean(effectiveChannels.fanpage)}, Groups (Nhóm): ${Boolean(effectiveChannels.groups)}`);
  console.log(`=================================================`);
...
export async function executeSingleNode(nodeType, payload = {}, config = null) {
  ...
  switch (nodeType) {
    ...
    case 'chatgpt': {
      ...
      const rawRatio = payload.aspectRatio || cfg.aspectRatio || '16:9';
      const ratio = (rawRatio === '4:5' || rawRatio === '4/5') ? '16:9' : rawRatio;
